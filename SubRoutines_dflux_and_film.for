	!
	!   UNITS OF THE PROBLEM : Ton,mm,N,s,deg C
...
C	  !!! Abaqus variables
	  ...
	  !!! Model variables
	  real(8) :: pad_number_of_nodes, total_nb_nodes
C		
C	  !!! Physical variables
	  real(8) :: a,b,c
	  real(8) :: tire_radius,friction_coef,N
C	  
...
C	  !!! Declaring the variables for flux definition  !!!!
	  real(8) :: rotational_speed_i
	  real(8) :: radius_M
C	  
C	  !!! To enter :
	  tire_radius = 320
...
C	  
	  ...
	  !------------------------------------------------!
	  !!!!!!! GET VARIABLES FORM THE VARIABLES FILE !!!!!!
	  !------------------------------------------------!
	  
	  !Defined by the "from_xlsx_to_for.py" file, do not modify
	  ...
	  IF ((Node_coordinates_rotated(1) .LE. (b/2))  .AND. (Node_coordinates_rotated(1) .GE. (-b/2))) THEN
		  IF ((Node_coordinates_rotated(2) .LE. (a+c)) .AND. (Node_coordinates_rotated(2) .GE. (c))) THEN
			  ...
			  radius_M = ((X**2 + Y**2)**(0.5))
			  rotational_speed_i = ((speed_i*1000)/3.6)/tire_radius !s-1
			  FLUX(1) = brake_i*exchange_coef*friction_coef*N*rotational_speed_i*radius_M/(a*b)
			  
			  !Flux in N.mm.s-1, in  Ton.mm2.s-3
...
C	  !!! Abaqus variables
	  ...
	  !!!! Declaring variables for the command read !!!! -> Get Angle,Speed,Brake
	  real(8) :: angle_i, speed_i, brake_i
	  COMMON /PHYBLOCK/ angle_i, speed_i, brake_i
	  
	  !------------------------------------------------!
    !!!!!!! SET THE CONVECTIVE COEEFICIENT FOR THE NODE !!!!!!
	  !------------------------------------------------!	  
	  
	  ! in mm
	  
	  ! H in W/mm-2 .K)
	  H(1) = -(0.00000255*(speed_i**4)-0.000615*(speed_i**3) + 0.0445*(speed_i**2) - 1.308*speed_i - 2.302)/(1000000)
	  RETURN
      END